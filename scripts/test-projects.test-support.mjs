...
const VITEST_RUN_POOL_ENV_KEY = "OPENCLAW_VITEST_RUN_POOL";
...
export function applyVitestRunPoolOverrideToArgs(args, env = process.env) {
  const pool = env[VITEST_RUN_POOL_ENV_KEY]?.trim();
  if (pool !== "forks" && pool !== "threads") {
    return args;
  }
  if (
    args.some((arg) => typeof arg === "string" && (arg === "--pool" || arg.startsWith("--pool=")))
  ) {
    return args;
  }
  return [...args, "--pool", pool];
}

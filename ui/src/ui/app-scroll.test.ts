...
describe("handleChatScroll", () => {
  it("sets chatUserNearBottom=true when within the 180px threshold", () => {
  ...
  it("sets chatUserNearBottom=true when distance is just under threshold", () => {
    const { host } = createScrollHost({});
    // distanceFromBottom = 2000 - 1421 - 400 = 179 → just under threshold
    const event = createScrollEvent(2000, 1421, 400);
    handleChatScroll(host, event);
    expect(host.chatUserNearBottom).toBe(true);
  });
  ...
  it("sets chatUserNearBottom=false when distance is exactly at threshold", () => {
    const { host } = createScrollHost({});
    // distanceFromBottom = 2000 - 1420 - 400 = 180 → at threshold (uses strict <)
    const event = createScrollEvent(2000, 1420, 400);
    handleChatScroll(host, event);
  ...
  it("sets chatUserNearBottom=false when user scrolled up past one long message", () => {
    const { host } = createScrollHost({});
    // distanceFromBottom = 2000 - 1380 - 400 = 220 → intentionally outside the tighter threshold
    const event = createScrollEvent(2000, 1380, 400);
    handleChatScroll(host, event);
    expect(host.chatUserNearBottom).toBe(false);
  });

  it("clears the new-messages flag once the user scrolls back near the bottom", () => {
    const { host } = createScrollHost({});
    ...
    const nearBottomEvent = createScrollEvent(2000, 1450, 400);
    handleChatScroll(host, nearBottomEvent);

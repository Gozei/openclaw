...
export async function runPreparedReply(
  ...
  const hasCurrentTurnMediaAttachment = hasInboundMedia(ctx) || (opts?.images?.length ?? 0) > 0;
  const hasMediaAttachment = hasInboundMedia(sessionCtx) || hasCurrentTurnMediaAttachment;
  ...
  const threadContextNote = hasCurrentTurnMediaAttachment
    ? undefined
    : threadHistoryBody
